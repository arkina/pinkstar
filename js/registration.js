$(document).ready(function() {
	...
	$("#empregs").submit(function(){
		...
		var pattern = /^\d{10}$/; var pattern_phone = /^\d{14}$/;
		if($("#fname").val()==""){
	...
	//	}
		if($("#address-one").val()==""){
			...
			$("#alterno_per").focus();
			return false;
		}if (!pattern_phone.test($("#alterno_per").val())) {
			$("#alterno_per_error").html("Mobile should be 14 digits");
			  return false;
		}else {
			$("#alterno_per_error").html(" ");
		}if($("#mobileno_per").val()==""){
			$("#mobileno_per_error").html("Field should not be blank");
			$("#mobileno_per").focus();
			return false;
		}else{
			$("#mobileno_per_error").html(" ");
		}if(isNaN($("#mobileno_per").val())){
			$("#mobileno_per_error").html("Enter only numeric value");
			$("#mobileno_per").focus();
			return false;
		}if (!pattern.test($("#mobileno_per").val())) {
			$("#mobileno_per_error").html("Mobile should be 10 digits");
			  return false;
		}else {
			$("#mobileno_per_error").html(" ");
		}if($("#alterno_res").val()==""){
			$("#alterno_res_error").html("Field should not be blank");
			$("#alterno_res").focus();
			return false;
		}else{
			$("#alterno_res_error").html(" ");
		}if(isNaN($("#alterno_res").val())){
			$("#alterno_res_error").html("Enter only numeric value");
			$("#alterno_res").focus();
			return false;
		}if (!pattern_phone.test($("#alterno_res").val())) {
			$("#alterno_res_error").html("Mobile should be 14 digits");
			  return false;
		}else {
			$("#alterno_res_error").html(" ");
		}if($("#mobileno_res").val()==""){
			$("#mobileno_res_error").html("Field should not be blank");
			$("#mobileno_res").focus();
			return false;
		}else{
			$("#mobileno_res_error").html(" ");
		}if(isNaN($("#mobileno_res").val())){
			$("#mobileno_res_error").html("Enter only numeric value");
			$("#mobileno_res").focus();
			return false;
		}if (!pattern.test($("#mobileno_res").val())) {
			$("#mobileno_res_error").html("Mobile should be 10 digits");
			  return false;
		}else {
			$("#mobileno_res_error").html(" ");
...
$("#depart").change(function(){
	var getDepart_id = $(this).val();
	if(getDepart_id!="" && getDepart_id!= "undefined"){
		var url ='http://pinkstarapp.com/administrator/';
		$.ajax({  
			type: 'POST',  
			url: url+'getcountry.php', 
			data: {getDepart_id:getDepart_id,checkdata:1},
			dataType : "html",
			success: function(data) {
