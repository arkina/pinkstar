	...
	if(!empty($_REQUEST)){
	  ...
	  if($_REQUEST['stepno']==1){
		...
		if($_REQUEST['stepno']==2){
			...
			if($_REQUEST['addmore']=='Add More'){
				if($_REQUEST['first_name']!="" && $_REQUEST['alternate_mobile']!=""){
					#echo "<pre>";echo "2";print_r($_REQUEST);die;
					$temp = array();
				foreach ($_REQUEST as $key => $value){
					if(($key!=""  && $value!="")){
						if($key!= 'PHPSESSID' && $key!="stepno" && $key!="addmore"){
							if($key=="vendor_id"){
				...
				$query="select * from ps_vendor_contact_person where vendor_id='".base64_decode($_REQUEST['vendor_id'])."'";
					$getData=$objCurd->CheckDataExists_id($query);
						if($getData){
							header("Location: ../edit-vendor-commision.php?formid=".$_REQUEST['vendor_id']."");
							exit();
						}else{
						$_SESSION['success']="Insert at least one record";
						header("Location: ../edit-vendor-contact.php?formid=".$_REQUEST['vendor_id']."");
							exit();
						}
				#echo "<pre>";echo "2";print_r($_REQUEST);die;
