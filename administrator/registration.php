...
if(!empty($_REQUEST)){
$temp_login =array("fname","lname","email","username","userpassword","region","depart","designation","doj",'post_position','date_exist');
$temp_personal =array('dob','marital_status','address-one','address-two','alterno_per','mobileno_per','alterno_res','mobileno_res','country','city','state','pincode');
...
foreach ($_REQUEST as $key => $value){
	if(($key!=""  && $value!="")){
		if(in_array($key,$temp_login)){
			...
			$user_id = $id;
			$login_array['doj']=date('Y-m-d H:i:s');
			...
			$referal_array['referral_created']=date('Y-m-d H:i:s');
...
//echo "<pre>";	print_r($_FILES);die;
	if(!empty($_FILES) && !empty($_FILES['userimage_name']['type']) || $_FILES['userimage_name']['type']!=""){
